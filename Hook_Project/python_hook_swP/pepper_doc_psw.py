...
class Houpub:
    ...
    def login(host, identification, password):
        ...
        Args: string
            ...
            identification: "pipeline@rapa.org"
    ...
    def add_task_to_entity(self, task_type_name):
        """
        (asset or shont) entity 를 할당하여 해당 asset or shot 의 새로운 task를 만든다
        Args:
    ...
    def publish_working_file(self, task_type_name, software_name):
        """
        (ex) publish_working_file("simulation", "hou")
        * need self parameter 순서 중요 : project,asset,entity
                                                or
                                      project,seq,shot,entity

        1. task_type_name 으로  task(dict)를 get
        2. software_name 으로 software(dict) get
    ...
    def publish_output_file(self, task_type_name, output_type_name, comments):
        """
        (ex) publish_output_file('FX', 'Movie_file', "test")
        * need self parameter 순서 중요 : project,asset,entity
                                                or
                                      project,seq,shot,entity

        1. task_type_name 으로  task_type(dict),task(dict)를 get
        2. last working file(dict) get
        3. output_type_name 으로 output type get
    ...
    def working_file_path(self, task_type_name, software_name, input_num):
        ...
        (ex) working_file_path("simulation", "hou", 10)
        ...
        * need self parameter 순서 중요 : project,asset,entity
        ...
        Returns:
            (path) 원하는 revision working file path
    ...
    def output_file_path(self, output_type_name, task_type_name, input_num):
        """
        output file path 를 리턴한다.

        revision = (intput_num < last revision)
        input num 이 크면 last revision 을 반환, 작으면 input_num 반환

        (ex) output_file_path('Movie_file', 'FX', 4)
    ...
    def make_next_output_path(self, output_type_name, task_type_name):
        ...
        (ex) make_next_output_path('Movie_file', 'FX')
        ...
        * need self parameter 순서 중요 : project,asset,entity
        ...
        Returns:
            (path) output file revision +1 된  path
    ...
    def get_revision_num(revision_max, input_num):
        ...
        Args:
            revision_max: (int0
            input_num: (int)
        Returns: (int) input num 이 크면 last revision 을 반환, 작으면 input_num 반환
        """
    ...
    def get_task(self, task_type_name):
        """
        task 와 task type 을 get 하는 함수 이다.
